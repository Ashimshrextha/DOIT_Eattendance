...
{
   public class HREmployeeBulkAttendanceApprovedServices : BaseRepository<HREmployeeManualAttendance, HREmployeeManualAttendanceModel>, IHREmployeeManualAttendanceServices<HREmployeeManualAttendance>
    {
        public HREmployeeBulkAttendanceApprovedServices(IUnitOfWork unitOfWork) : base(unitOfWork)
        {
            if (unitOfWork == null)
            {
        ...
        public async Task<IPagedList<proc_GetEmployeeBulkAttendance_Result>> GetEmployeeList(long? idHRCompany, int pageNumber, int pageSize, string orderingBy, string orderingDirection)
        {
            try
            {
                object[] obj =
                ...
                return (await ExecuteProcedure<proc_GetEmployeeBulkAttendance_Result>("EXEC proc_GetEmployeeListByCompany @paramIdHRCompany", obj))
                     .OrderBy(orderingBy + " " + orderingDirection).ToPagedList(pageNumber, pageSize);
            }
            catch (Exception exp)
            {
                throw new Exception(exp.Message);
